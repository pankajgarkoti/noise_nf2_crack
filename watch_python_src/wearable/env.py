...
LOG_TAG = 'persimwear.env'
logger = logging.getLogger(LOG_TAG)
logger.setLevel(LOG_LVL)

def service_env_get(input):
    rpc = global_var.get('rpc')
    values = rpc.exec_svc(1, "svc_env_get", bytearray(json.dumps(input), encoding="utf8"), need_ack=False, need_rsp=True, timeout=3)
