...
def urpc_basic_sample_for_ble(rpc):
    # 注册本地服务
    ...
    output = rpc.exec_svc(1, "data_test", input, need_ack=False, need_rsp=True, timeout=10)
    logger.info(output.decode('utf-8'))
    # 执行远端服务 json_test
    input = {"arg1": 1, "arg2": "string", "arg3": True}
    input = bytearray(json.dumps(input), encoding="utf8")
    output = rpc.exec_svc(1, "json_test", input, need_ack=False, need_rsp=True, timeout=10)
    logger.info(output.decode('utf-8'))


def urpc_ffi_args_sample_for_ble():
    u8 = Arg(U8, 0xAA)
...
def urpc_ffi_svc_sample_for_ble(rpc):
    a = Arg(U8, 0x45)
    b = Arg(U8, 0x54)
    # 执行远端 ffi 函数 u8_sum
